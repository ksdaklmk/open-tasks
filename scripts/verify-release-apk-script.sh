...
home="$test_root/home"
build_tools="$home/Library/Android/sdk/build-tools/1.0.0"
...
expect_status_without_leak() {
    ...
    local label="$2"
    shift 2
    local output normalized_output status
    set +e
    output="$("$@" 2>&1)"
    status=$?
    set -e
    ...
    normalized_output="$(printf '%s' "$output" | tr 'A-F' 'a-f')"
    case "$normalized_output" in
        *"$matching_actual"*|*"$mismatching_actual"*)
