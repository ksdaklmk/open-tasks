...
| T14 | Dependency or CI workflow compromise | Minimal `google()`/`mavenCentral()` repositories, read-only CI token, no CI secrets, automated dependency update proposals | GitHub Actions are tag-pinned rather than commit-pinned; pin before CI receives signing or OAuth secrets |
