...
| Privacy policy | `https://ksdaklmk.github.io/open-tasks/privacy/` | URL fixed; public deployment remains PENDING |
| Support | `https://ksdaklmk.github.io/open-tasks/support/` | URL fixed; public deployment remains PENDING |
| Open Tasks account | None; the local workspace requires no account | Fixed from shipped flow |
| Developer legal/display identity | Not present in repository evidence | PENDING — owner/Console evidence |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| Task titles and bodies | `core/data/src/main/kotlin/app/opentasks/core/data/db/Entities.kt`; backup/export codecs | Stored in the encrypted local vault; used for task views, search and planning | Only through an enabled encrypted backup/package/archive or an explicit plaintext CSV/Markdown/report export | Google Drive app-data; Android backup/transfer; user-selected document provider/app | Local-only by default; optional off-device paths require current Play classification | No developer-directed sharing shown; provider/user-directed exceptions require current Play wording | Local processing required for the feature; every off-device path optional | App functionality; optional backup/recovery or user export | Drive uses HTTPS and encrypted objects; Android/provider transport varies by path | Delete in app or clear app data/uninstall; delete remote history for Drive; user deletes exported files at destination | PENDING — final AAB audit | PENDING — final AAB audit |
...
| Projects | `Entities.kt`; `WorkspaceCsvWriter.kt`; `ProjectMarkdownWriter.kt`; backup codecs | Stored and organised in the encrypted local vault | Optional encrypted backup/archive/package; explicit CSV, Markdown or report export is plaintext | Same backup recipients; user-selected document destination | Same source position as task content | Same source position as task content | Required locally when used; off-device paths optional | Project organisation; backup/recovery; user export | Same path-dependent result as task content | Same local/Drive/export deletion paths as task content | PENDING — final AAB audit | PENDING — final AAB audit |
...
| Schedules and dates | task/project entities; `feature/schedule`; backup/export codecs | Dates and planning state are processed locally in the vault and UI | Optional encrypted backup/archive/package; may appear in explicit plaintext exports | Same backup recipients; user-selected document destination | Same source position as task content | Same source position as task content | Required locally when used; off-device paths optional | Scheduling; backup/recovery; user export | Same path-dependent result as task content | Same local/Drive/export deletion paths as task content | PENDING — final AAB audit | PENDING — final AAB audit |
...
|---|---|---|---|---|
| Encrypted Google Drive backup | User explicitly connects Drive; pending changes enqueue a 15-minute-debounced run and a retained periodic check runs every 24 hours with network, battery and storage constraints | Authenticated encrypted vault objects and attachment chunks to the user's private Drive app-data space | Drive API HTTPS; scope is exactly `https://www.googleapis.com/auth/drive.appdata`; Drive file IDs, names, roles and bounded app properties are provider metadata | Delete history/attachment content removes owned remote objects; disconnect revokes authorization and clears local configuration but is not described as remote deletion. PENDING — final AAB audit |
| Drive account binding | Explicit account authorization or later silent authorization for an enabled backup | Google account/grant data through Play Services; Drive permission ID from `about.get`; per-install HMAC digest retained locally | Access token and account handle are session-only in app source; raw permission ID/email/profile are not persisted | Disconnect attempts scope revocation; clear app data removes the digest. PENDING — final AAB audit |
| Android backup package | User prepares the package; Android performs eligible cloud backup/device transfer | Exactly `files/android_backup/open_tasks_portable_v1.otb` | Android 12+ cloud backup requires encryption capabilities; device transfer includes the package. Legacy `fullBackupContent` excludes every domain, so it does not broaden the package | Reprepare/withdraw package or clear app data; platform-held copy follows Android/Google controls. PENDING — final AAB audit |
...
| OAuth scope | Production authorization requests one scope: `https://www.googleapis.com/auth/drive.appdata` | PASS — repository source audit; owner Cloud configuration and final AAB remain PENDING |
...
| Backup | `allowBackup=true`; Android 12+ extraction rules include only the encrypted portable package and require cloud encryption capability; legacy rules exclude all domains | PASS — repository manifest/rules audit; runtime restore remains PENDING |
| Exported components | Main activity is exported for launcher, quick add, text share and process-text entry; Quick Settings tile service is exported behind `BIND_QUICK_SETTINGS_TILE`; other declared receivers/providers are not exported | PASS — repository manifest audit; PENDING — final merged-manifest audit |
...
| Optional Google authorization | Used only for encrypted backup/recovery in private Drive app-data; core app remains available without it | Drive authorization manager, lifecycle coordinator and More backup UI | PASS — repository evidence; owner OAuth/Console configuration PENDING |
...
| Exported activity/service | Main activity handles launcher, quick add and user-directed incoming text; tile service exposes quick add only to the system under `BIND_QUICK_SETTINGS_TILE` | Manifest intent filters and permission | PASS — repository evidence; PENDING — final merged-manifest audit |
| Android backup | Allows only the prepared encrypted portable package on Android 12+ cloud backup/device transfer; legacy rules exclude all domains | Manifest and both backup-rule XML files | PASS — repository evidence; device qualification PENDING |
| Privacy policy | Fixed in-app and listing URL | More → Privacy policy opens `https://ksdaklmk.github.io/open-tasks/privacy/` | Repository link PASS; public deployment and Console entry PENDING |
